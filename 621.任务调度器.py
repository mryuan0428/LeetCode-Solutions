...
class Solution:
    def leastInterval(self, tasks: List[str], n: int) -> int:
        ...
        counter = {}
        for t in tasks:
            if t in counter: counter[t] += 1
            else: counter[t] = 1

        task_maxnum = 0
        for m in counter.values():
            if m > task_maxnum: task_maxnum = m
        maxtask_num = sum(1 for t in counter if counter[t] == task_maxnum)

        return max((task_maxnum-1)*(n+1)+maxtask_num , len(tasks))
